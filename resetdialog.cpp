    ...
    ui(new Ui::ResetDialog)
{
    ...
    //连接reset相关信号和注册处理回调
    initHandlers();
    connect(HttpMgr::getInstance().get(), &HttpMgr::signal_reset_mod_finish, this,
            &ResetDialog::slot_reset_mod_finish);

    ui->label_pwd_visible->setState("unvisible", "unvisible_hover", "",
                                     "visible", "visible_hover", "");
    ui->pwd_edit->setEchoMode(QLineEdit::Password);
    connect(ui->label_pwd_visible, &ClickedLabel::clicked, this, [=](){
        if (ui->label_pwd_visible->getCurState() == Normal) {
            ui->pwd_edit->setEchoMode(QLineEdit::Password);
        } else {
...
{
    qDebug() << "slot_reset_mod_finish id is " << id;
...
void ResetDialog::on_sure_btn_clicked()
{
    ...
    QByteArray enpwd = QCryptographicHash::hash(ui->pwd_edit->text().toUtf8(), QCryptographicHash::Sha256).toHex();
    json_obj["passwd"] = QJsonValue(QString::fromUtf8(enpwd));
