    ...
    ui(new Ui::ApplyFriendPage)
{
    ui->setupUi(this);
    //???
    connect(ui->listWidget_apply_friend, &ApplyFriendList::sig_show_search, this, &ApplyFriendPage::sig_show_search);
    loadApplyList();
    //接受tcp传递的authrsp信号处理
...
void ApplyFriendPage::addNewApply(std::shared_ptr<AddFriendApply> apply)
{
    ...
    auto apply_info = std::make_shared<ApplyInfo>(apply->m_from_uid,
                                                  apply->m_name, apply->m_desc,heads[head_i], apply->m_name, 0, 0);
    apply_item->setInfo( apply_info);
    QListWidgetItem* item = new QListWidgetItem;
    //qDebug()<<"chat_user_wid sizeHint is " << chat_user_wid->sizeHint();
    item->setSizeHint(apply_item->sizeHint());
    item->setFlags(item->flags() & ~Qt::ItemIsEnabled & ~Qt::ItemIsSelectable);
    ui->listWidget_apply_friend->insertItem(0,item);
...
void ApplyFriendPage::loadApplyList()
{
    ...
    for(int i = 0; i < 13; i++){
        ...
        item->setSizeHint(apply_item->sizeHint());
        item->setFlags(item->flags() & ~Qt::ItemIsEnabled & ~Qt::ItemIsSelectable);
        ui->listWidget_apply_friend->addItem(item);
        ui->listWidget_apply_friend->setItemWidget(item, apply_item);
        //收到审核好友信号
        connect(apply_item, &ApplyFriendItem::sig_auth_friend, [this](std::shared_ptr<ApplyInfo> apply_info){
            // auto *authFriend =  new AuthenFriend(this);
